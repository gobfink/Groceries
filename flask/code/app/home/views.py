...
from .forms import AcronymsForm, AcronymSearchForm, AddTagForm, GroceryForm
...
@home.route('/groceries/add', methods=['GET', 'POST'])
# @login_required
def add_grocery():
    """
    ...
    """
    q_quality = db_quality.query.all()
    q_stores = db_store.query.all()
    form = GroceryForm()

#    flash(q_quality)
#    flash(q_stores)
    qualities=[(i.id, i.name) for i in q_quality]
    stores=[(i.id, i.name) for i in q_stores]
    form.quality.choices = qualities
    form.store.choices = stores

    if form.submit.data:
        if form.validate_on_submit():
            # Will need to look at this ( i think i'm missing some backend pieces to make this work )
            ...
            groceryToAdd = db_Grocery(name=form.name.data,
                          ...
                          ounces=form.ounces.data,
                          date=datetime.datetime.now(),
                          author_id=authid)

            db.session.add(groceryToAdd)
            db.session.commit()
 ...
 #           db.session.commit()
            ...
            flash('You have successfully added a new grocery - \'' + form.name.data + '\'')
            return redirect(url_for('home.groceries'))
        else:
            flash('Fill in required fields!')
    if form.cancel.data:
        flash('You have cancelled the add of a new Grocery')
        return redirect(url_for('home.groceries'))
